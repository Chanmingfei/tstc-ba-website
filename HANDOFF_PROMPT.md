...
11. **弹窗关闭按钮 + 夜间弹窗质感统一（2026-08-04 续四）**：① 弹窗关闭按钮（`#searchModalClose` / `#changelogClose` / `#closeModal` / `#closeFeedbackModal`）与顶栏圆型按钮统一液态玻璃：夜间为 `rgba(255,255,255,.1)` frosted 白 + `rgba(255,255,255,.14)` 边框 + 内外阴影；日间为 `rgba(0,0,0,.05)` 浅灰 frost 以适应白色弹窗面板。② 夜间弹窗面板去白色光圈：背景从 `rgba(18,22,30,.88)` 改为 `rgba(30,34,44,.72)`，边框从 `.55` 降到 `.12`，顶部内发光从 `.6` 降到 `.12`，并改用柔和大投影；搜索弹窗输入框夜间从 `bg-gray-50` 白底改为 `var(--surface-2)` 暗色。
12. **灯箱 + 分享 FAB + 评分链接卡统一（2026-08-04 续五）**：① 灯箱按钮与夜间顶栏圆钮统一为 `rgba(255,255,255,.1)` 平铺雾化白 + 柔和阴影；灯箱右下角描述卡改为深色半透玻璃（`rgba(14,16,21,.62)`）并加 `text-shadow`，确保亮色图片背景下文字可读。② 修复夜间分享浮标子按钮不透明的回归：删除两条覆盖规则，子按钮与分享主按钮 / 返回顶部统一使用夜间同款雾化白玻璃。③ 新生指南系列中「学校食堂 / 附近商场 / 体育课评分合集」等外链卡改为 `.glass-link` 液态玻璃卡片，标题用 `.glass-link-title` 跟随主题变量，不再使用 `bg-white` 实心白卡。

12. **新增新生指南（十一）-吧主谈社团组织（2026-08-05）**：post-16 / post-16-en，按指南系列统一格式重新排版、润色，配图（百团大战现场、到梦空间 APP 截图）与到梦空间学分模块表；社团评分外链采用 `.glass-link` 液态玻璃卡片，与指南三体育课评分合集风格一致。新增文章模板流程：复制 post-8 模板 → 改 `#articleMeta` / `<title>` / `<meta description>` / 文章头部硬编码标题和日期 → 替换 `.article-content` → `npm run build`。
...
> 仓库已克隆到 `/workspace/tstc-ba-website`，请接手。背景：唐山师范学院吧官网静态双语站，Cloudflare Pages 部署。近期已做全站暗色模式 + 昼夜滑动开关、QZone 图标去白底、全站液态玻璃（Liquid Glass）设计（含 2026-08-04 多轮强化：统一左上镜面高光、搜索框玻璃 + 搜索按钮液态玻璃化、导航栏玻璃顶边、顶栏通透度提升、灯箱控制按钮与描述卡片液态玻璃化、弹窗关闭按钮与顶栏圆型按钮统一为液态玻璃、夜间弹窗面板柔化去白色光圈、夜间按钮边缘与光影柔化、顶栏圆型按钮夜间默认改为被选中态的平铺雾化白、灯箱按钮与夜间顶栏统一质感且描述卡加可读性、分享 FAB 子按钮暗色透明度修复、评分链接卡玻璃化）、移动端顶栏单行修复、README 配图重写；发文章流程见上文第 4/5 节。注意：① 暗色模式与液态玻璃改组件时看§12 的易错点；② 推送只走 `git push origin main`（Gitee），别试 GitHub；③ 改 `main.js`/`tailwind-input.css` 后必须 `npm run build`。
