...
- **2026-08-04 液态玻璃质感再强化 + 风格统一**：提升玻璃令牌（透明度、高光边框、内发光、阴影、模糊 / 饱和度），为所有玻璃表面统一添加左上镜面高光；首页头部搜索框改为玻璃药丸，导航栏增加顶部玻璃高光边线，平台色按钮进一步提亮高光，已玻璃化的按钮 / 卡片 / 弹窗折射感更明显；进一步下调顶栏背景透明度（日间 .58 / 夜间 .42）提升通透度，搜索框内按钮透明化统一为同一块玻璃，夜间模式按钮边缘与光影全面柔化，顶栏圆型按钮夜间默认质感统一为「被选中」的平铺雾化白。
