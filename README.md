...
_redirects              路由规则：把所有不存在的路径重写为自定义 404 页面
...
> 自定义 404：`_redirects` 中配置了通配重写规则 `/* /404 200`，任何不存在的路径都会展示根目录下的 `404.html` 自定义页面（已有页面与静态资源不受影响）。
> 注：Cloudflare Pages 的 `_redirects` 仅支持 200 重写（不支持 404 状态码），因此缺失链接当前以 200 状态展示 404 页面；如需严格返回 404 状态码，需在 Cloudflare 控制台用 Transform Rules / Bulk Redirects 或 Pages Functions 实现。
